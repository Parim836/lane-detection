...
class LaneNetPostProcessor(object):
    ...
    #         'source_image': source_image,
    #     }
    def postprocess(self, binary_seg_result, instance_seg_result, source_image, with_lane_fit=False, data_source='tusimple'):
        mask_image = np.zeros_like(source_image, np.uint8)

        # ===== ตรงนี้คือจุดที่คุณสามารถแทน clustering ด้วย OpenCV ได้ =====
        binary_seg_result = np.array(binary_seg_result * 255, dtype=np.uint8)

        num_labels, labels, stats, _ = cv2.connectedComponentsWithStats(binary_seg_result, connectivity=8)
        min_area = 150
        filtered = np.zeros_like(binary_seg_result)
